...
async def save_activity_log(
    ...
    _safe_doctor_id = doctor_id if doctor_id and _UUID_RE.match(doctor_id) else None
    ...
    try:
        ...
        if _safe_doctor_id:
            await increment_doctor_cases(_safe_doctor_id)
...
async def book_appointment(
    ...
    _UUID_RE2 = re.compile(r'^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$', re.I)
    _safe_doc_id = doctor_id if doctor_id and _UUID_RE2.match(doctor_id) else None
    try:
        patient = await get_or_create_patient(patient_phone)
        patient_id = patient.get("id")
        appt = await create_appointment({
            "patient_id": patient_id,
            "doctor_id": _safe_doc_id,
